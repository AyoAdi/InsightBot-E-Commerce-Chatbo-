...
# # run_agent.py - This script runs your AI agent from the Colab Terminal with plotting capability
# 
# # --- Phase 0: Initial Setup and Imports ---
# # Install necessary libraries (these will run every time the script is executed)
# import subprocess
# import sys
# 
...
# 
# 
# # --- Modified ask_llm_for_sql to include plotting intent ---
...
# 
#         # Refined parsing logic (same as before)
...
#             generated_text = generated_text[len("```"):].strip()
# 
#         # Extract plot intent before cleaning SQL
#         plot_intent = False
...
#                  sql_query += ';'
# 
#         # Append ##PLOT## back if it was detected
#         if plot_intent:
#             sql_query += "##PLOT##"
# 
#         return sql_query
...
# 
# 
# # --- Modified format_answer to include plotting logic ---
# # --- Modified format_answer to include more robust plotting logic ---
...
#     Generates and saves a plot if plot_filename is provided and data is suitable.
#     """
#     if result is None:
...
# 
#     # If plot_filename is provided, attempt to create a plot
#     if plot_filename:
...
#                 # Attempt to convert columns to appropriate types for plotting
#                 for col in df_plot.columns:
#                     # Try converting to numeric first
#                     df_plot[col] = pd.to_numeric(df_plot[col], errors='ignore')
#                     # Then try converting to datetime if it's an object type
...
# 
#                 # Basic plotting logic: assume first column is X, second is Y
#                 x_col = column_names[0]
#                 y_col = column_names[1]
# 
#                 plt.figure(figsize=(10, 6))
# 
#                 # Try to infer plot type based on converted dtypes
...
#                     # Line plot for time series or scatter for two numerics
#                     if pd.api.types.is_datetime64_any_dtype(df_plot[x_col]):
#                         df_plot = df_plot.sort_values(by=x_col) # Sort by date for line plot
#                         plt.plot(df_plot[x_col], df_plot[y_col], marker='o', linestyle='-')
#                         plt.xticks(rotation=45, ha='right')
#                     else: # Assuming numeric X for scatter/bar
#                         # If X is categorical/discrete numeric, use bar plot
#                         if df_plot[x_col].nunique() < 20 and not pd.api.types.is_float_dtype(df_plot[x_col]): # Heuristic for categorical X
#                             plt.bar(df_plot[x_col].astype(str), df_plot[y_col])
#                             plt.xticks(rotation=45, ha='right')
#                         else: # Otherwise, a scatter or line plot for continuous numeric X
#                             plt.plot(df_plot[x_col], df_plot[y_col], marker='o', linestyle='-')
#                             plt.xticks(rotation=45, ha='right') # Still rotate for readability
# 
# 
...
# 
# 
#     # Original non-plotting formatting
...
#     plot_filename = None
#     if plot_requested:
#         plot_filename = "generated_plot.png" # Define a filename for the plot
# 
...
#     print("Examples: 'What is my total sales?', 'Plot total sales by date.'") # Updated examples
#     print("---------------------------------------\n")
# 
#     while True:
